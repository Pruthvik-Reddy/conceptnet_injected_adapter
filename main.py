...
def load_pretrained_model(args):
    ...
    """"
    # Pretrained Model
    bert = BertForMaskedLM.from_pretrained(args.bert_model)
    num_adapters = bert.config.num_hidden_layers
    adapter_names=[]
    for i in range(num_adapters):
        adapter_name = f"adapter_{i}"
        adapter_names.append(adapter_name)
        bert.add_adapter(adapter_name)
    bert.set_active_adapters(adapter_names)
    ...
    if "albert" in args.bert_model:
        ...
        bert.roberta.embeddings.token_type_embeddings = nn.Embedding(
    ...
    bert._init_weights(bert.roberta.embeddings.token_type_embeddings)
...
def save_model(args, model, tokenizer):
    ...
    torch.save(model_to_save.state_dict(), './saved_models/adapter_model_7.pth')
...
def load_trained_model(args, model, tokenizer):
    ...
    """
    if hasattr(model, "module"):
        model.module.load_state_dict(torch.load('./saved_models/adapter_model_7.pth'))
    else:
        model.load_state_dict(torch.load('./saved_models/adapter_model_7.pth'))
